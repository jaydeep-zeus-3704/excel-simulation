...
export class Formula {
    ...
    static evaluate(value: string, store: CellStore): string {
        ...
        try {
            ...
            // MIN(A1:A10)
            const minMatch = expression.match(
                /^MIN\(([A-Z]+\d+):([A-Z]+\d+)\)$/i
            );
            ...
            // MAX(A1:A10)
            const maxMatch = expression.match(
                /^MAX\(([A-Z]+\d+):([A-Z]+\d+)\)$/i
            );
    ...
    private static getCellValue(
        ...
        const parsed =
            this.parseCellReference(cellRef);
    ...
    private static getRangeValues(
        ...
        const start =
            this.parseCellReference(startRef);

        const end =
            this.parseCellReference(endRef);
